...
type_t CCS_BLOCK(fir)(type_t input, type_t taps[NUM_TAPS]) {
    ...
    static type_t delay_lane[NUM_TAPS] = {}; // needs to maintain state between calls
...
//     MAIN: for (int i = 0; i < NUM_SAMPLES; i++) {

//         static type_t delay_lane[NUM_TAPS] = {};

//         type_t input = in_channel.read();
...
// void CCS_BLOCK(test)(ac_channel<type_t> &in_channel, ac_channel<type_t> &out_channel, type_t taps[NUM_TAPS]) {
//     fir(in_channel, out_channel, taps);
// }
